...
from BankWithDatabase.DatabaseMethods import change_pin_in_database
...
class Ui_change_pin_window(object):
    ...
    def enter_pressed(self, user_index, change_pin_window):
        ...
        if len(input_pin) == 0 and len(confirm_pin) == 0:
            ...
            return False
        elif input_pin == get_account()[user_index][2]:
            self.same_pin_error()
            return False
            ...
            elements[user_index][2] = confirm_pin
            edit_account(elements)
            change_pin_in_database(elements[user_index][1], confirm_pin)
